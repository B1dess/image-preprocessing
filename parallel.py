...
def image_preprocessing(input_name, n_workers):
    ...
    for key, value in steps.items():
        ...
        plt.axis('off')
        plt.show()
        path = fr'C:\Users\yurch\OneDrive\Documents\GitHub\image-preprocessing\output_parallel\{input_name}'
        if not os.path.exists(path):
            os.makedirs(path)
        cv2.imwrite(f"output_parallel/{input_name}/{i}_{key}.jpg", step)
        i += 1
