...
def image_preprocessing(input_name, n_workers):
    ...
    for i in range(20):
        ...
        print(f"{input_name} - {n_workers} процесів - Час виконання [{i}]: {execution_time:.2f} мс")
    print(f"{input_name} - {n_workers} процесів - Середній час виконання: {np.mean(exec_times):.2f} мс")


if __name__ == "__main__":
    images = ["bird", "gihun", "large"]
    n = [2, 4, 6, 8, 10, 12]
    for input_image in images:
        for n_worker in n:
            image_preprocessing(input_image, n_worker)